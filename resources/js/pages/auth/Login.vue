...
<div class="p-4 mt-4 pt-0 flex items-center justify-center bg-cover bg-center">
    ...
    <div class="bg-white dark:bg-gray-800 shadow w-full rounded-lg divide-y divide-gray-200">
        ...
        <form @submit.prevent='authenticate'>
            <label class="font-semibold text-sm dark:text-gray-200 text-gray-600 pb-1 block">E-mail</label>
            <input required v-model="form.email" type="email" class="border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" />
            <label  class="font-semibold text-sm dark:text-gray-200 text-gray-600 pb-1 block">Password</label>
            <input required type="text" v-model="form.password" class="border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" />
            <button  type="submit" class="transition duration-200 bg-gray-500 hover:bg-gray-600 focus:bg-gray-700 focus:shadow-sm focus:ring-4 focus:ring-gray-500 focus:ring-opacity-50 text-white w-full py-2.5 rounded-lg text-sm shadow-sm hover:shadow-md font-semibold text-center inline-block">
                <span class="inline-block mr-2">{{loginText}}
        ...
        methods: {
            authenticate() {
                ...
                setTimeout(() => {
                    ...
                    .catch((error) => {
                        setTimeout(() => {
                            this.$store.dispatch('alert/success', error);
